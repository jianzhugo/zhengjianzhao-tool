...
async function importTransformers() {
  ...
  // 注意：registry.npmmirror.com 对本包不在 unpkg 白名单内（FORBIDDEN），不可用
  const esmUrls = [
...
async function segmentImage(img) {
  ...
  if (!__segPipe) {
    ...
    // 注意：registry.npmmirror.com 对 @huggingface/transformers 不在 unpkg 白名单内，
    // 所有文件请求均返回 FORBIDDEN，不可作为 WASM 源
    const WASM_BASES = [
